...
if [[ -n "$current_branch" ]]; then
  # Refresh remote-tracking refs before the unpushed-commit check.
  # The auto-merge workflow deletes the claude/* branch on the remote after
  # merging into main; without --prune, the stale origin/<branch> ref persists
  # locally and rev-list reports non-zero unpushed commits even though the
  # push succeeded and the workflow already consumed the branch. --prune drops
  # the dead ref so rev-parse below falls through to the origin/HEAD path.
  # Silent on failure (e.g. offline) so the hook remains usable without network.
