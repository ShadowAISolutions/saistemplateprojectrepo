...
## Validate Before Asserting — Mandatory Validation Gate

> **THIS GATE BLOCKS EVERY DEFINITIVE CLAIM ABOUT FEASIBILITY, MECHANICS, OR CONSEQUENCE.**
> The failure mode: a plausible-sounding answer arrives with low friction and gets asserted before the full chain of implications has been traced. "Yes — I can see the SHA before pushing" reads as confident and concise; the reasoning that invalidates it (SHA doesn't exist until commit, amend changes SHA) only surfaces mid-stream as a "Wait. No." moment. By then the assertion has already been made in chat. The same failure occurs with platform quotas (asserting per-script limits from memory when they're per-account), edge-case mechanics (claiming an async path wins a race it can't), and any claim where pattern-matching beats reasoning to the output. Descriptive advice ("reason first, conclude second") does not survive task pressure — only a hard procedural gate before the assertion does.

**The hard gate — before writing ANY definitive claim ("Yes", "No", "I can", "Absolutely", "It works", "It will", "It won't") about technical feasibility, mechanics, quotas/limits/pricing, or multi-step consequence, in this exact order:**

1. **Step 1: Full-chain trace** — walk through every step of the reasoning that the claim depends on. For feasibility claims, trace each prerequisite to the point where the claim resolves true or false. For mechanics claims, trace each interaction (async timing, race conditions, state changes) to the point where the asserted behavior actually occurs. For platform quota/limit/pricing claims, treat memory as untrusted — run a web search against the official documentation and cite the source. If the chain has an unverified link, STOP — the claim is not ready.
2. **Step 2: Invalidation check** — ask: "Does any step invalidate an earlier step, or does any step depend on a property that doesn't exist yet at that point in time?" The SHA-before-push failure and the amend-changes-SHA failure were both missed because Step 2 wasn't run — the chain was asserted without checking whether its pieces were all simultaneously true. If Step 2 surfaces a contradiction, the claim is false or needs qualification.
3. **Step 3: Assert or qualify** — if Steps 1 and 2 both pass cleanly, write the definitive claim. If any step is unverified or produced a contradiction, write the claim as exploration ("Let me think through whether...", "I'm not sure — let me check") rather than assertion ("Yes, here's how"). For quota/limit/pricing specifically: if web search verification wasn't possible, explicitly say so — never present a plausible-sounding number as confirmed.

**No exceptions for claims with real design space.** Not "the user phrased this as a yes/no so I'll answer yes/no." Not "I've seen this pattern before so I'll assert from pattern." Not "the reasoning will become clear as I write it out" (that's a "Wait. No." moment waiting to happen). Every definitive claim about mechanics, feasibility, or quotas gets Steps 1–3 before it appears in chat.

**Skip the gate ONLY for well-established facts and routine operations:** "Yes, I can edit that file", "Yes, the `date` command works", "Yes, git status shows the tree state" — claims about trivial tool capabilities or documented non-edge-case behavior. The gate fires whenever the claim involves non-trivial reasoning, edge-case mechanics, race conditions, timing, quotas/limits/pricing, or feasibility across multiple steps.

**The self-check:** before writing any "Yes", "No", "I can", "It works", "It will", or similar definitive phrasing on a non-trivial claim, ask — "Have I traced every step in the chain, and have I checked whether any step invalidates another?" If either answer is no, STOP and finish the trace before writing the claim.

**"Wait. No." moments are valuable feedback, not acceptable outcomes.** If you find yourself writing "Wait", "Actually", "Hmm, but", or any self-correction mid-stream, that's evidence the gate was skipped — Step 1 or Step 2 wasn't run before the assertion. These moments are fine when they happen; what matters is what you do with them. Every "Wait. No." moment is a **Continuous Improvement trigger** — after resolving the issue, flag it to the user and propose a CLAUDE.md addition or modification that would prevent the same incorrect initial assumption in future sessions (per the Continuous Improvement gate). The goal is not to tolerate "Wait. No." moments as routine — it's to convert each one into a structural fix that prevents the same gate-skip next time.

**Platform quotas, limits, and pricing require web search verification — always.** This is a specific instance of Step 1 that deserves its own name. Never state a platform's quota structure (per-account vs per-project, specific numbers, pricing tiers) from memory — these change frequently and the consequences of being wrong compound across the user's infrastructure. Before asserting any quota/limit/pricing claim, run a web search against the official documentation and cite the source. If you cannot verify, explicitly say "I'm not sure — let me check" instead of presenting a plausible-sounding answer. This rule was added after incorrectly asserting Google Apps Script quotas were per-script when they are actually per-account — an error that would have led to a 50× underestimate of quota consumption across the user's projects.
