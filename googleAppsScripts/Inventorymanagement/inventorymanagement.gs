...
function doGet(e) {
  ...
  // All visual UI is rendered by the HTML page (inventorymanagement.html).
...
// PROJECT OVERRIDE REMOVED: Old session page HTML template (CSS + body + JS, ~720 lines)
// was replaced by the worker RPC bridge above. Original code had inline styles, admin panel,
// version display, user email, and applyData() UI — all now in inventorymanagement.html.
// (See testauthhtml1.gs lines 2821-2872 for the same pattern.)
