...
function doGet(e) {
  ...
  // ── Page nonce validation (postMessage handshake flow) ──
  // The page_nonce is a one-time-use token generated by generatePageNonce()
  // after a successful postMessage handshake between the GAS iframe and the
  // embedding page. It binds a validated session to a single page load without
  // ever exposing the session token in the URL.
  var pageNonce = (e && e.parameter && e.parameter.page_nonce) || '';
  if (pageNonce) {
    sessionToken = validatePageNonce(pageNonce) || '';
  }

  // Normal flow: validate session token (from page_nonce or legacy ?session= param)
  var session = validateSession(sessionToken);
