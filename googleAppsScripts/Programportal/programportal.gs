...
function doGet(e) {
  ...
  // PROJECT: HIPAA Admin Panel — full self-contained UI served in iframe
  if (action === 'hipaaAdmin') {
    var hipaaAdminCss = ''
      + '* { box-sizing: border-box; margin: 0; padding: 0; }'
      + 'body { background: rgba(20,20,30,0.98); color: #ccc; font: 12px/1.4 monospace; overflow-y: auto; }'
      + '#hipaa-app { display: flex; flex-direction: column; height: 100vh; }'
      + '#hipaa-nav { display: flex; border-bottom: 1px solid #444; padding: 6px 8px; gap: 4px; flex-shrink: 0; }'
      + '.nav-tab { background: none; border: 1px solid #555; color: #90caf9; cursor: pointer; font: 10px/1 monospace; padding: 3px 8px; border-radius: 3px; }'
      + '.nav-tab:hover { background: rgba(144,202,249,0.1); }'
      + '.nav-tab.active { background: rgba(144,202,249,0.15); border-color: #90caf9; }'
      + '#hipaa-menu { display: flex; flex-wrap: wrap; gap: 4px; padding: 6px 8px; border-bottom: 1px solid #333; flex-shrink: 0; }'
      + '.menu-btn { background: none; border: none; color: #90caf9; cursor: pointer; font: 11px/1 monospace; padding: 2px 6px; }'
      + '.menu-btn:hover { color: #fff; text-decoration: underline; }'
      + '.menu-btn.active { color: #fff; font-weight: bold; }'
      + '#hipaa-content { flex: 1; overflow-y: auto; padding: 10px 12px; }'
      + '#hipaa-loading { text-align: center; color: #888; padding: 40px; font-size: 11px; }'
      + '.panel { display: none; }'
      ...
      + '.pa-close { background: none; border: none; color: #999; cursor: pointer; font-size: 16px; padding: 0 4px; }'
      + '.pa-close:hover { color: #fff; }';

    // HIPAA Admin Panel HTML — continued in next section (hipaaAdminHtml)
    var hipaaAdminHtml = ''
      + '<div id="hipaa-nav">'
      + '  <button class="nav-tab active" data-phase="A">Patient Rights</button>'
      + '  <button class="nav-tab" data-phase="B">Breach &amp; Reps</button>'
      + '  <button class="nav-tab" data-phase="C">Retention &amp; Legal</button>'
      + '</div>'
      + '<div id="hipaa-menu"></div>'
      + '<div id="hipaa-content">'
      + '  <div id="hipaa-loading">Waiting for authentication...</div>'
      + '</div>';

    // HIPAA Admin Panel JS
    var hipaaAdminJs = ''
      + 'var PARENT_ORIGIN = ' + JSON.stringify(PARENT_ORIGIN) + ';'
      + 'var _token = null;'
      ...
      + 'var _menuItems = [];'
      // ── Utility: HTML escape
      + 'function _esc(s) { if (!s) return ""; return String(s).replace(/&/g,"\\x26amp;").replace(/</g,"\\x26lt;").replace(/>/g,"\\x26gt;").replace(/"/g,"\\x26quot;"); }'
      // ── PostMessage: signal ready to parent
      + 'window.top.postMessage({type:"hipaa-admin-ready"}, PARENT_ORIGIN);'
      // ── PostMessage: receive token from parent
      + 'window.addEventListener("message", function(evt) {'
      + '  if (evt.origin !== PARENT_ORIGIN) return;'
      + '  if (!evt.data) return;'
      + '  if (evt.data.type === "hipaa-admin-init" && evt.data.token) {'
      + '    _token = evt.data.token;'
      + '    _loadMenu();'
      + '  }'
      + '  if (evt.data.type === "hipaa-admin-signout" || evt.data.type === "hipaa-admin-close") {'
      + '    _token = null;'
      + '    _activePanel = null;'
      + '    document.getElementById("hipaa-menu").innerHTML = "";'
      + '    document.getElementById("hipaa-content").innerHTML = "<div id=\\"hipaa-loading\\">Session ended.</div>";'
      + '  }'
      + '});'
      // ── Load menu from server (RBAC-filtered)
      + 'function _loadMenu() {'
      + '  document.getElementById("hipaa-loading").textContent = "Loading menu...";'
      + '  google.script.run'
      + '    .withSuccessHandler(function(r) {'
      + '      if (!r || !r.success) { document.getElementById("hipaa-loading").textContent = "Error loading menu."; return; }'
      + '      _menuItems = r.items;'
      + '      _buildMenu();'
      + '      document.getElementById("hipaa-loading").textContent = "Select a tool from the menu above.";'
      + '    })'
      + '    .withFailureHandler(function(e) {'
      + '      document.getElementById("hipaa-loading").textContent = "Error: " + String(e);'
      + '    })'
      + '    .getHipaaMenu(_token);'
      + '}'
      // ── Build menu buttons from items
      + 'function _buildMenu() {'
      + '  var menu = document.getElementById("hipaa-menu");'
      + '  menu.innerHTML = "";'
      + '  for (var i = 0; i < _menuItems.length; i++) {'
      + '    if (_menuItems[i].phase !== _activePhase) continue;'
      + '    var btn = document.createElement("button");'
      + '    btn.className = "menu-btn";'
      + '    btn.textContent = _menuItems[i].label;'
      + '    btn.title = _menuItems[i].description;'
      + '    btn.setAttribute("data-id", _menuItems[i].id);'
      + '    btn.addEventListener("click", (function(item) { return function() { _openPanel(item.id); }; })(_menuItems[i]));'
      ...
      + '    _activePhase = this.getAttribute("data-phase");'
      + '    _buildMenu();'
      + '  });'
      + '}'
      // ── Panel open/close
      ...
      + '  for (var b = 0; b < btns.length; b++) btns[b].classList.toggle("active", btns[b].getAttribute("data-id") === panelId);'
      + '  var content = document.getElementById("hipaa-content");'
      + '  content.innerHTML = _getPanelHtml(panelId);'
      + '  _attachPanelListeners(panelId);'
      + '  _loadPanelData(panelId);'
      + '}'
      // ── Panel HTML templates
      + 'function _getPanelHtml(id) {'
      + '  switch(id) {'
      + '    case "disclosures": return \'<div class="pa-header"><span class="pa-title">Disclosure Accounting</span><span><select id="disclosure-export-format" style="background:rgba(255,255,255,0.08);border:1px solid #555;color:#ccc;font:10px monospace;padding:1px 4px;border-radius:3px;"><option value="json">JSON</option><option value="csv">CSV</option></select><button id="disclosure-export-btn" class="pa-action">Export</button></span></div><div style="padding:4px 0;"><label style="display:inline;color:#aaa;font-size:11px;cursor:pointer;"><input type="checkbox" id="disclosure-grouped-toggle" checked /> Group repeated disclosures</label></div><div id="disclosure-period" class="pa-status"></div><div id="disclosure-list"></div><div id="disclosure-empty" class="pa-empty" style="display:none;">No disclosures found for the reporting period.</div>\';'
      + '    case "data-export": return \'<div class="pa-header"><span class="pa-title">Download My Data</span></div><div class="pa-body"><p style="color:#aaa;font-size:11px;margin:0 0 8px;">Download a copy of all your data stored in this system. This is your right under HIPAA §164.524.</p><div class="pa-format-picker"><label><input type="radio" name="export-format" value="json" checked> JSON</label><label><input type="radio" name="export-format" value="csv"> CSV</label><label><input type="radio" name="export-format" value="summary"> Summary</label></div><div id="summary-agreement" style="display:none;margin:6px 0;padding:6px;background:rgba(255,255,255,0.04);border-radius:4px;"><p style="color:#aaa;font-size:10px;margin:0 0 4px;">Per HIPAA §164.524(c)(3), you are requesting a summary instead of the complete record.</p><label style="font-size:10px;display:inline;cursor:pointer;"><input type="checkbox" id="summary-agree-checkbox" /> I agree to receive a summary</label></div><button id="data-export-download-btn" class="pa-action" style="margin-top:8px;">Download</button><div id="data-export-status" class="pa-status"></div></div>\';'
      + '    case "amendment": return \'<div class="pa-header"><span class="pa-title">Request Record Correction</span></div><div class="pa-body"><p style="color:#aaa;font-size:11px;margin:0 0 8px;">Under HIPAA §164.526, you have the right to request corrections.</p><label>Record to correct:<input type="text" id="amend-record-id" placeholder="Record ID or description" /></label><label>Current content:<textarea id="amend-current" rows="2" placeholder="The existing content"></textarea></label><label>Proposed correction:<textarea id="amend-proposed" rows="3" placeholder="Your proposed correction"></textarea></label><label>Reason:<textarea id="amend-reason" rows="2" placeholder="Why this correction is needed"></textarea></label><button id="amend-submit-btn" class="pa-action" style="margin-top:8px;">Submit Request</button><div id="amend-status" class="pa-status"></div></div>\';'
      ...
      + '    case "disagreement": return \'<div class="pa-header"><span class="pa-title">Statement of Disagreement</span></div><div class="pa-body"><p style="color:#aaa;font-size:11px;margin:0 0 8px;">If your amendment request was denied, you have the right under §164.526(d) to file a statement of disagreement.</p><label>Amendment Request ID:<input type="text" id="disagree-amendment-id" placeholder="ID of the denied amendment" /></label><label>Your statement:<textarea id="disagree-statement" rows="4" placeholder="Your statement of disagreement"></textarea></label><button id="disagree-submit-btn" class="pa-action" style="margin-top:8px;">Submit Statement</button><div id="disagree-status" class="pa-status"></div></div>\';'
      + '    case "extension": return \'<div class="pa-header"><span class="pa-title">Grant 30-Day Extension</span></div><div class="pa-body"><p style="color:#aaa;font-size:11px;margin:0 0 8px;">HIPAA permits one 30-day extension for access or amendment requests.</p><label>Request type:<select id="extension-type" style="background:rgba(255,255,255,0.08);border:1px solid #555;color:#ccc;font:12px monospace;padding:4px;border-radius:4px;width:100%;"><option value="access">Access Request (§164.524)</option><option value="amendment">Amendment Request (§164.526)</option></select></label><label>Request ID:<input type="text" id="extension-request-id" placeholder="Original request ID" /></label><label>Reason:<textarea id="extension-reason" rows="3" placeholder="Written explanation for the delay"></textarea></label><button id="extension-submit-btn" class="pa-action" style="margin-top:8px;">Grant Extension</button><div id="extension-status" class="pa-status"></div></div>\';'
      + '    case "denial-notice": return \'<div class="pa-header"><span class="pa-title">Formal Denial Notice</span></div><div class="pa-body"><p style="color:#aaa;font-size:11px;margin:0 0 8px;">Generate a formal written denial notice per §164.524(d)(2).</p><label>Request Type:<select id="denial-request-type" style="background:rgba(255,255,255,0.08);border:1px solid #555;color:#ccc;font:11px monospace;padding:3px 6px;border-radius:3px;width:100%;"><option value="access">Access Request</option><option value="amendment">Amendment Request</option></select></label><label>Request/Amendment ID:<input type="text" id="denial-request-id" placeholder="Request identifier" /></label><label>Basis for denial:<textarea id="denial-basis" rows="3" placeholder="Specific legal basis for denying this request"></textarea></label><label style="display:inline;color:#aaa;font-size:11px;cursor:pointer;"><input type="checkbox" id="denial-reviewable" checked /> Denial is reviewable</label><label>Contact person:<input type="text" id="denial-contact" value="HIPAA Privacy Officer" /></label><label>Contact office:<input type="text" id="denial-office" value="Privacy Office" /></label><button id="denial-generate-btn" class="pa-action" style="margin-top:8px;">Generate Notice</button><div id="denial-status" class="pa-status"></div><div id="denial-notice-output" style="display:none;margin-top:8px;padding:8px;background:rgba(255,255,255,0.04);border-radius:4px;font-size:11px;color:#ccc;white-space:pre-wrap;"></div></div>\';'
      + '    case "ehr-disclosures": return \'<div class="pa-header"><span class="pa-title">EHR Disclosure Accounting (HITECH)</span><span><button id="ehr-refresh-btn" class="pa-action">Refresh</button></span></div><div class="pa-body"><div id="ehr-period" class="pa-status"></div><div id="ehr-list"></div><div id="ehr-empty" class="pa-empty" style="display:none;">No EHR TPO disclosures found for the 3-year reporting period.</div></div>\';'
      + '    case "breach-log": return \'<div class="pa-header"><span class="pa-title">Breach Log &amp; Reporting</span><span><button id="breach-refresh-btn" class="pa-action">Refresh</button></span></div><div class="pa-body"><div id="breach-log-list"></div><div id="breach-empty" class="pa-empty" style="display:none;">No breaches logged.</div><div style="margin-top:10px;border-top:1px solid #333;padding-top:10px;"><span style="color:#aaa;font-size:11px;font-weight:bold;">Log New Breach</span><label>Description:<textarea id="breach-description" rows="2" placeholder="Brief description"></textarea></label><label>Nature of PHI:<input type="text" id="breach-nature-phi" placeholder="e.g., Clinical notes" /></label><label>Affected count:<input type="number" id="breach-affected-count" min="0" value="0" /></label><label>Mitigation:<textarea id="breach-mitigation" rows="2" placeholder="Actions taken"></textarea></label><button id="breach-log-submit-btn" class="pa-action" style="margin-top:8px;">Log Breach</button><div id="breach-log-status" class="pa-status"></div></div><div style="margin-top:10px;border-top:1px solid #333;padding-top:10px;"><span style="color:#aaa;font-size:11px;font-weight:bold;">Annual Report</span><label>Year:<input type="number" id="breach-report-year" placeholder="e.g. 2025" style="width:80px;" /></label><button id="breach-report-btn" class="pa-action">Generate Report</button><div id="breach-report-result"></div></div></div>\';'
      + '    case "representatives": return \'<div class="pa-header"><span class="pa-title">Personal Representatives</span><span><button id="rep-refresh-btn" class="pa-action">Refresh</button></span></div><div class="pa-body"><div id="rep-list"></div><div id="rep-empty" class="pa-empty">No representatives registered.</div><div style="margin-top:10px;border-top:1px solid #333;padding-top:10px;"><span style="color:#aaa;font-size:11px;font-weight:bold;">Register Representative</span><label>Representative Email:<input type="email" id="rep-email" placeholder="representative@example.com" /></label><label>Individual Email:<input type="email" id="rep-individual-email" placeholder="patient@example.com" /></label><label>Relationship Type:<select id="rep-relationship"><option value="">Select...</option><option value="Parent">Parent</option><option value="LegalGuardian">Legal Guardian</option><option value="HealthcarePOA">Healthcare POA</option><option value="CourtAppointed">Court Appointed</option><option value="Executor">Executor</option></select></label><label>Expiration (optional):<input type="date" id="rep-expiration" /></label><label>Document Ref:<input type="text" id="rep-document-ref" placeholder="Auth doc reference" /></label><label>Notes:<input type="text" id="rep-notes" placeholder="Additional notes" /></label><button id="rep-register-btn" class="pa-action" style="margin-top:8px;">Register</button><div id="rep-register-status" class="pa-status"></div></div></div>\';'
      + '    case "legal-holds": return \'<div class="pa-header"><span class="pa-title">Legal Holds</span><span><button id="lh-refresh-btn" class="pa-action">Refresh</button></span></div><div class="pa-body"><div style="margin-bottom:8px;"><label style="color:#aaa;font-size:11px;">Filter by Status:<select id="lh-status-filter" style="font-size:11px;background:#1a1a2e;color:#ccc;border:1px solid #555;border-radius:3px;padding:2px 4px;"><option value="">All</option><option value="Active" selected>Active</option><option value="Released">Released</option><option value="Expired">Expired</option></select></label></div><div id="lh-list"></div><div id="lh-empty" class="pa-empty" style="display:none;">No legal holds found.</div><div style="margin-top:10px;border-top:1px solid #333;padding-top:10px;"><span style="color:#aaa;font-size:11px;font-weight:bold;">Place New Legal Hold</span><label>Sheet:<select id="lh-sheet-name"><option value="">Select...</option><option value="SessionAuditLog">SessionAuditLog</option><option value="DataAuditLog">DataAuditLog</option><option value="DisclosureLog">DisclosureLog</option><option value="AccessRequests">AccessRequests</option><option value="AmendmentRequests">AmendmentRequests</option><option value="AmendmentNotifications">AmendmentNotifications</option><option value="BreachLog">BreachLog</option><option value="PersonalRepresentatives">PersonalRepresentatives</option><option value="LegalHolds">LegalHolds</option><option value="RetentionIntegrityLog">RetentionIntegrityLog</option></select></label><label>Hold Type:<select id="lh-hold-type"><option value="">Select...</option><option value="Litigation">Litigation</option><option value="Regulatory">Regulatory</option><option value="InternalInvestigation">Internal Investigation</option><option value="Audit">Audit</option><option value="Preservation">Preservation</option></select></label><label>Reason:<textarea id="lh-reason" rows="2" placeholder="Legal basis"></textarea></label><label>Case Ref:<input type="text" id="lh-case-ref" placeholder="Case or docket number" /></label><label>Custodian:<input type="email" id="lh-custodian" placeholder="custodian@example.com" /></label><label>Start Date:<input type="date" id="lh-start-date" /></label><label>End Date:<input type="date" id="lh-end-date" /></label><label>Expiration:<input type="date" id="lh-expiration" /></label><button id="lh-place-btn" class="pa-action" style="margin-top:8px;">Place Hold</button><div id="lh-place-status" class="pa-status"></div></div></div>\';'
      + '    case "compliance-audit": return \'<div class="pa-header"><span class="pa-title">Retention Compliance Audit</span><span><button id="ca-run-btn" class="pa-action">Run Audit</button></span></div><div class="pa-body"><div id="ca-result"></div><div id="ca-empty" class="pa-empty">Click "Run Audit" to check retention compliance.</div><div style="margin-top:10px;border-top:1px solid #333;padding-top:10px;"><span style="color:#aaa;font-size:11px;font-weight:bold;">Export Report</span><div class="pa-format-picker"><label><input type="radio" name="ca-export-format" value="json" checked /> JSON</label><label><input type="radio" name="ca-export-format" value="text" /> Text</label></div><button id="ca-export-btn" class="pa-action">Export</button><div id="ca-export-result"></div></div></div>\';'
      + '    case "archive-integrity": return \'<div class="pa-header"><span class="pa-title">Archive Integrity Verification</span><span><button id="ai-verify-btn" class="pa-action">Verify All</button></span></div><div class="pa-body"><div id="ai-result"></div><div id="ai-empty" class="pa-empty">Click "Verify All" to recompute and compare archive checksums.</div></div>\';'
      + '    case "retention-policy": return \'<div class="pa-header"><span class="pa-title">Retention Policy Documentation</span><span><button id="rp-generate-btn" class="pa-action">Generate</button></span></div><div class="pa-body"><div id="rp-result"></div><div id="rp-empty" class="pa-empty">Click "Generate" to create a formal retention policy document.</div><div style="margin-top:10px;border-top:1px solid #333;padding-top:10px;"><span style="color:#aaa;font-size:11px;font-weight:bold;">Export Policy</span><div class="pa-format-picker"><label><input type="radio" name="rp-export-format" value="text" checked /> Text</label><label><input type="radio" name="rp-export-format" value="json" /> JSON</label></div><button id="rp-export-btn" class="pa-action">Export</button><div id="rp-export-result"></div></div></div>\';'
      + '    default: return "<div class=\\"pa-empty\\">Unknown panel.</div>";'
      + '  }'
      + '}'
      // ── Data loading (triggers google.script.run on panel open)
      ...
      + '}'
      // ── GAS call helper
      + 'function _gasCall(fn, args, onSuccess, onError) {'
      + '  var runner = google.script.run'
      + '    .withSuccessHandler(onSuccess || function(){})'
      + '    .withFailureHandler(onError || function(e) { console.error("GAS error:", e); });'
      ...
      + '    _gasCall("getGroupedDisclosureAccounting", [_token], function(r) { _renderDisclosures(r); });'
      + '  } else {'
      + '    _gasCall("getDisclosureAccounting", [_token], function(r) { _renderDisclosures(r); });'
      + '  }'
      + '}'
      ...
      + '  _gasCall("getPendingAmendments", [_token], function(r) { _renderPendingAmendments(r); });'
      + '}'
      ...
      + '  _gasCall("getDisclosureAccounting", [_token, null, {includeEhrTpo: true}], function(r) { _renderEhrDisclosures(r); });'
      + '}'
      ...
      + '  _gasCall("getBreachLog", [_token], function(r) { _renderBreachLog(r); });'
      + '}'
      ...
      + '  _gasCall("getPersonalRepresentatives", [_token], function(r) { _renderRepresentatives(r); });'
      + '}'
      ...
      + '  _gasCall("getLegalHolds", [_token, filter ? {status: filter.value} : {}], function(r) { _renderLegalHolds(r); });'
      + '}'
      // ── Render functions
      ...
      + '  if (!result || !result.success) { if(container) container.innerHTML = "<div style=\\"color:#ef5350\\">Error: " + _esc(result ? result.message : "No response") + "</div>"; return; }'
      ...
      + '    html += "<div class=\\"pa-card\\"><div class=\\"pa-card-header\\">" + _esc(d.recipient || "Unknown") + "</div>";'
      + '    html += "<div class=\\"pa-card-meta\\">" + _esc(d.date || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-field\\"><strong>Purpose:</strong> " + _esc(d.purpose || "") + "</div>";'
      + '    if (d.description) html += "<div class=\\"pa-card-field\\">" + _esc(d.description) + "</div>";'
      + '    if (d.count) html += "<div class=\\"pa-card-field\\"><strong>Occurrences:</strong> " + _esc(d.count) + "</div>";'
      + '    html += "</div>";'
      + '  }'
      ...
      + '    var d = result.items[i];'
      + '    html += "<div class=\\"pa-card\\"><div class=\\"pa-card-header\\">" + _esc(d.recipient || "Unknown") + "</div>";'
      + '    html += "<div class=\\"pa-card-meta\\">" + _esc(d.date || "") + " — " + _esc(d.purpose || "TPO") + "</div>";'
      + '    if (d.description) html += "<div class=\\"pa-card-field\\">" + _esc(d.description) + "</div>";'
      + '    html += "</div>";'
      + '  }'
      + '  if(container) container.innerHTML = html;'
      ...
      + '    html += "<div class=\\"pa-card\\"><div class=\\"pa-card-header\\">Amendment #" + _esc(a.id || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-meta\\">Requested: " + _esc(a.date || "") + " by " + _esc(a.requestor || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-field\\"><strong>Record:</strong> " + _esc(a.recordId || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-field\\"><strong>Proposed:</strong> " + _esc(a.proposed || "") + "</div>";'
      + '    if (a.reason) html += "<div class=\\"pa-card-field\\"><strong>Reason:</strong> " + _esc(a.reason) + "</div>";'
      + '    html += "<div class=\\"pa-card-actions\\">";'
      + '    html += "<button class=\\"pa-approve\\" data-amend-id=\\"" + _esc(a.id) + "\\">Approve</button>";'
      + '    html += "<button class=\\"pa-deny\\" data-amend-id=\\"" + _esc(a.id) + "\\">Deny</button>";'
      + '    html += "<textarea class=\\"pa-deny-reason\\" data-amend-id=\\"" + _esc(a.id) + "\\" placeholder=\\"Reason for denial\\"></textarea>";'
      + '    html += "</div></div>";'
      + '  }'
      + '  if(container) container.innerHTML = html;'
      ...
      + '      _gasCall("reviewAmendment", [_token, id, "Approved", ""], function() { _loadPendingAmendments(); });'
      + '    });'
      + '  }'
      ...
      + '      _gasCall("reviewAmendment", [_token, id, "Denied", reason], function() { _loadPendingAmendments(); });'
      + '    });'
      + '  }'
      + '}'
      ...
      + '    html += "<div class=\\"pa-card\\"><div class=\\"pa-card-header\\">Breach #" + _esc(b.id || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-meta\\">Discovered: " + _esc(b.discoveryDate || "") + " | Status: " + _esc(b.status || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-field\\">" + _esc(b.description || "") + "</div>";'
      + '    if (b.count) html += "<div class=\\"pa-card-field\\"><strong>Affected:</strong> " + _esc(b.count) + " individuals</div>";'
      + '    html += "</div>";'
      + '  }'
      + '  if(container) container.innerHTML = html;'
      + '}'
      ...
      + '    html += "<div class=\\"pa-card\\"><div class=\\"pa-card-header\\">" + _esc(r.name || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-meta\\">" + _esc(r.email || "") + " — " + _esc(r.relationship || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-field\\"><strong>Patient:</strong> " + _esc(r.patientEmail || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-actions\\"><button class=\\"pa-deny\\" data-rep-id=\\"" + _esc(r.id) + "\\">Revoke</button></div>";'
      + '    html += "</div>";'
      + '  }'
      + '  if(container) container.innerHTML = html;'
      ...
      + '      _gasCall("revokeRepresentative", [_token, repId, "Revoked by admin"], function() { _loadRepresentatives(); });'
      + '    });'
      + '  }'
      + '}'
      ...
      + '    html += "<div class=\\"pa-card\\"><div class=\\"pa-card-header\\">" + _esc(h.caseName || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-meta\\">Placed: " + _esc(h.date || "") + " | Custodian: " + _esc(h.custodian || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-field\\"><strong>Scope:</strong> " + _esc(h.scope || "") + "</div>";'
      + '    html += "<div class=\\"pa-card-actions\\"><button class=\\"pa-approve\\" data-hold-id=\\"" + _esc(h.id) + "\\">Release</button></div>";'
      + '    html += "</div>";'
      + '  }'
      + '  if(container) container.innerHTML = html;'
      ...
      + '      var holdId = this.getAttribute("data-hold-id");'
      + '      _gasCall("releaseLegalHold", [_token, holdId, "Released by admin"], function() { _loadLegalHolds(); });'
      + '    });'
      + '  }'
      + '}'
      // ── Simple result renderers (audit, integrity, policy)
      + 'function _renderSimpleResult(elId, result, field) {'
      + '  var el = document.getElementById(elId);'
      + '  if (!el) return;'
      + '  if (!result || !result.success) { el.innerHTML = "<div style=\\"color:#ef5350\\">Error: " + _esc(result ? result.message : "No response") + "</div>"; return; }'
      + '  el.innerHTML = "<div class=\\"pa-card\\" style=\\"white-space:pre-wrap;font-size:10px;\\">" + _esc(result[field] || "") + "</div>";'
      + '}'
      ...
      + 'function _attachPanelListeners(id) {'
      + '  switch(id) {'
      // Disclosures panel
      + '    case "disclosures":'
      + '      var groupToggle = document.getElementById("disclosure-grouped-toggle");'
      + '      if (groupToggle) groupToggle.addEventListener("change", function() { _loadDisclosures(); });'
      + '      var expBtn = document.getElementById("disclosure-export-btn");'
      + '      if (expBtn) expBtn.addEventListener("click", function() {'
      + '        var fmt = document.getElementById("disclosure-export-format").value;'
      + '        _gasCall("exportDisclosureAccounting", [_token, fmt], function(r) { _downloadResult(r, "hipaa-disclosures." + fmt); });'
      + '      });'
      + '      break;'
      // Data export panel
      + '    case "data-export":'
      ...
      + '          var sa = document.getElementById("summary-agreement");'
      + '          if (sa) sa.style.display = this.value === "summary" ? "" : "none";'
      + '        });'
      + '      }'
      + '      var dlBtn = document.getElementById("data-export-download-btn");'
      + '      if (dlBtn) dlBtn.addEventListener("click", function() {'
      + '        var fmt = document.querySelector("input[name=\\"export-format\\"]:checked").value;'
      + '        if (fmt === "summary") {'
      + '          var cb = document.getElementById("summary-agree-checkbox");'
      + '          if (!cb || !cb.checked) { _setStatus("data-export-status", "Please agree to receive a summary.", "#ef5350"); return; }'
      + '          _gasCall("generateDataSummary", [_token], function(r) { _downloadResult(r, "hipaa-summary.json"); _setStatus("data-export-status", "Summary ready.", "#66bb6a"); });'
      + '        } else {'
      + '          _gasCall("requestDataExport", [_token, fmt], function(r) { _downloadResult(r, "hipaa-export." + fmt); _setStatus("data-export-status", "Export ready.", "#66bb6a"); });'
      + '        }'
      + '      });'
      + '      break;'
      // Amendment request panel
      + '    case "amendment":'
      + '      var subBtn = document.getElementById("amend-submit-btn");'
      + '      if (subBtn) subBtn.addEventListener("click", function() {'
      + '        var recordId = document.getElementById("amend-record-id").value.trim();'
      + '        var current = document.getElementById("amend-current").value.trim();'
      + '        var proposed = document.getElementById("amend-proposed").value.trim();'
      + '        var reason = document.getElementById("amend-reason").value.trim();'
      + '        if (!recordId || !proposed) { _setStatus("amend-status", "Record ID and proposed correction required."); return; }'
      + '        _setStatus("amend-status", "Submitting...");'
      + '        _gasCall("requestAmendment", [_token, recordId, current, proposed, reason], function(r) {'
      + '          if (r && r.success) { _setStatus("amend-status", "Submitted. ID: " + (r.id || "N/A"), "#66bb6a"); } else { _setStatus("amend-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      break;'
      // Amendment review panel
      + '    case "amendment-review":'
      + '      var refBtn = document.getElementById("amend-review-refresh");'
      + '      if (refBtn) refBtn.addEventListener("click", function() { _loadPendingAmendments(); });'
      + '      break;'
      // Disagreement panel
      + '    case "disagreement":'
      + '      var disBtn = document.getElementById("disagree-submit-btn");'
      + '      if (disBtn) disBtn.addEventListener("click", function() {'
      + '        var amendId = document.getElementById("disagree-amendment-id").value.trim();'
      + '        var statement = document.getElementById("disagree-statement").value.trim();'
      + '        if (!amendId || !statement) { _setStatus("disagree-status", "Amendment ID and statement required."); return; }'
      + '        _setStatus("disagree-status", "Submitting...");'
      + '        _gasCall("submitDisagreement", [_token, amendId, statement], function(r) {'
      + '          if (r && r.success) { _setStatus("disagree-status", "Disagreement filed.", "#66bb6a"); } else { _setStatus("disagree-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      break;'
      // Extension panel
      + '    case "extension":'
      + '      var extBtn = document.getElementById("extension-submit-btn");'
      + '      if (extBtn) extBtn.addEventListener("click", function() {'
      + '        var extType = document.getElementById("extension-type").value;'
      + '        var reqId = document.getElementById("extension-request-id").value.trim();'
      + '        var reason = document.getElementById("extension-reason").value.trim();'
      + '        if (!reqId || !reason) { _setStatus("extension-status", "Request ID and reason required."); return; }'
      + '        _setStatus("extension-status", "Submitting...");'
      + '        var fn = extType === "amendment" ? "requestAmendmentExtension" : "requestAccessExtension";'
      + '        _gasCall(fn, [_token, reqId, reason], function(r) {'
      + '          if (r && r.success) { _setStatus("extension-status", "Extension granted. New deadline: " + (r.newDeadline || "N/A"), "#66bb6a"); } else { _setStatus("extension-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      break;'
      // Denial notice panel
      + '    case "denial-notice":'
      + '      var denBtn = document.getElementById("denial-generate-btn");'
      + '      if (denBtn) denBtn.addEventListener("click", function() {'
      + '        var reqId = document.getElementById("denial-request-id").value.trim();'
      + '        if (!reqId) { _setStatus("denial-status", "Request ID required."); return; }'
      ...
      + '        _gasCall("generateDenialNotice", [_token, document.getElementById("denial-request-type").value, reqId, params], function(r) {'
      + '          if (r && r.success) { _setStatus("denial-status", "Notice generated.", "#66bb6a"); var out = document.getElementById("denial-notice-output"); if(out) { out.style.display = ""; out.textContent = r.notice || ""; } }'
      + '          else { _setStatus("denial-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      break;'
      // EHR disclosures panel
      + '    case "ehr-disclosures":'
      + '      var ehrRef = document.getElementById("ehr-refresh-btn");'
      + '      if (ehrRef) ehrRef.addEventListener("click", function() { _loadEhrDisclosures(); });'
      + '      break;'
      // Breach log panel
      + '    case "breach-log":'
      + '      var brRef = document.getElementById("breach-refresh-btn");'
      + '      if (brRef) brRef.addEventListener("click", function() { _loadBreachLog(); });'
      + '      var brSub = document.getElementById("breach-log-submit-btn");'
      + '      if (brSub) brSub.addEventListener("click", function() {'
      ...
      + '        _gasCall("logBreach", [_token, {description: desc, phiNature: document.getElementById("breach-nature-phi").value.trim(), count: document.getElementById("breach-affected-count").value, mitigation: document.getElementById("breach-mitigation").value.trim()}], function(r) {'
      + '          if (r && r.success) { _setStatus("breach-log-status", "Breach logged. ID: " + (r.id || "N/A"), "#66bb6a"); _loadBreachLog(); } else { _setStatus("breach-log-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      var brRpt = document.getElementById("breach-report-btn");'
      + '      if (brRpt) brRpt.addEventListener("click", function() {'
      + '        var year = document.getElementById("breach-report-year").value;'
      + '        document.getElementById("breach-report-result").innerHTML = "<div class=\\"pa-status\\">Generating...</div>";'
      + '        _gasCall("getBreachReport", [_token, year || ""], function(r) {'
      + '          var el = document.getElementById("breach-report-result");'
      + '          if (!r || !r.success) { el.innerHTML = "<div style=\\"color:#ef5350\\">Error: " + _esc(r ? r.message : "No response") + "</div>"; }'
      + '          else { el.innerHTML = "<div class=\\"pa-card\\" style=\\"white-space:pre-wrap;font-size:10px;\\">" + _esc(r.report || "No breaches to report.") + "</div>"; }'
      + '        });'
      + '      });'
      + '      break;'
      // Representatives panel
      + '    case "representatives":'
      + '      var rpRef = document.getElementById("rep-refresh-btn");'
      + '      if (rpRef) rpRef.addEventListener("click", function() { _loadRepresentatives(); });'
      + '      var rpReg = document.getElementById("rep-register-btn");'
      + '      if (rpReg) rpReg.addEventListener("click", function() {'
      + '        var repEmail = document.getElementById("rep-email").value.trim();'
      + '        var indEmail = document.getElementById("rep-individual-email").value.trim();'
      + '        if (!repEmail || !indEmail) { _setStatus("rep-register-status", "Both emails required."); return; }'
      + '        _setStatus("rep-register-status", "Registering...");'
      + '        _gasCall("registerPersonalRepresentative", [_token, {repEmail: repEmail, individualEmail: indEmail, relationship: document.getElementById("rep-relationship").value, expiration: document.getElementById("rep-expiration").value, documentRef: document.getElementById("rep-document-ref").value.trim(), notes: document.getElementById("rep-notes").value.trim()}], function(r) {'
      + '          if (r && r.success) { _setStatus("rep-register-status", "Registered.", "#66bb6a"); _loadRepresentatives(); } else { _setStatus("rep-register-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      break;'
      // Legal holds panel
      + '    case "legal-holds":'
      + '      var lhRef = document.getElementById("lh-refresh-btn");'
      + '      if (lhRef) lhRef.addEventListener("click", function() { _loadLegalHolds(); });'
      + '      var lhFilter = document.getElementById("lh-status-filter");'
      + '      if (lhFilter) lhFilter.addEventListener("change", function() { _loadLegalHolds(); });'
      + '      var lhPlace = document.getElementById("lh-place-btn");'
      + '      if (lhPlace) lhPlace.addEventListener("click", function() {'
      + '        var sheet = document.getElementById("lh-sheet-name").value;'
      + '        var holdType = document.getElementById("lh-hold-type").value;'
      + '        if (!sheet || !holdType) { _setStatus("lh-place-status", "Sheet and hold type required."); return; }'
      + '        _setStatus("lh-place-status", "Placing hold...");'
      + '        _gasCall("placeLegalHold", [_token, {sheetName: sheet, holdType: holdType, reason: document.getElementById("lh-reason").value.trim(), caseRef: document.getElementById("lh-case-ref").value.trim(), custodian: document.getElementById("lh-custodian").value.trim(), startDate: document.getElementById("lh-start-date").value, endDate: document.getElementById("lh-end-date").value, expiration: document.getElementById("lh-expiration").value}], function(r) {'
      + '          if (r && r.success) { _setStatus("lh-place-status", "Hold placed. ID: " + (r.id || "N/A"), "#66bb6a"); _loadLegalHolds(); } else { _setStatus("lh-place-status", "Error: " + (r ? r.message : "No response"), "#ef5350"); }'
      + '        });'
      + '      });'
      + '      break;'
      ...
      + '      if (caRun) caRun.addEventListener("click", function() {'
      + '        document.getElementById("ca-result").innerHTML = "<div class=\\"pa-status\\">Running audit...</div>";'
      + '        var caEmpty = document.getElementById("ca-empty"); if(caEmpty) caEmpty.style.display = "none";'
      + '        _gasCall("auditRetentionCompliance", [_token], function(r) { _renderSimpleResult("ca-result", r, "report"); });'
      + '      });'
      + '      var caExp = document.getElementById("ca-export-btn");'
      + '      if (caExp) caExp.addEventListener("click", function() {'
      + '        var fmt = document.querySelector("input[name=\\"ca-export-format\\"]:checked").value;'
      + '        _gasCall("getComplianceAuditReport", [_token, fmt], function(r) { _downloadResult(r, "audit-report." + fmt); var el = document.getElementById("ca-export-result"); if(el) el.innerHTML = "<div style=\\"color:#66bb6a\\">Report downloaded.</div>"; });'
      + '      });'
      + '      break;'
      // Archive integrity panel
      + '    case "archive-integrity":'
      + '      var aiBtn = document.getElementById("ai-verify-btn");'
      + '      if (aiBtn) aiBtn.addEventListener("click", function() {'
      + '        document.getElementById("ai-result").innerHTML = "<div class=\\"pa-status\\">Verifying archives...</div>";'
      + '        var aiEmpty = document.getElementById("ai-empty"); if(aiEmpty) aiEmpty.style.display = "none";'
      + '        _gasCall("verifyArchiveIntegrity", [_token], function(r) { _renderSimpleResult("ai-result", r, "report"); });'
      + '      });'
      + '      break;'
      // Retention policy panel
      + '    case "retention-policy":'
      + '      var rpGen = document.getElementById("rp-generate-btn");'
      + '      if (rpGen) rpGen.addEventListener("click", function() {'
      + '        document.getElementById("rp-result").innerHTML = "<div class=\\"pa-status\\">Generating policy...</div>";'
      + '        var rpEmpty = document.getElementById("rp-empty"); if(rpEmpty) rpEmpty.style.display = "none";'
      + '        _gasCall("getRetentionPolicyDocument", [_token], function(r) { _renderSimpleResult("rp-result", r, "policy"); });'
      + '      });'
      + '      var rpExp = document.getElementById("rp-export-btn");'
      + '      if (rpExp) rpExp.addEventListener("click", function() {'
      + '        var fmt = document.querySelector("input[name=\\"rp-export-format\\"]:checked").value;'
      + '        _gasCall("exportRetentionPolicy", [_token, fmt], function(r) { _downloadResult(r, "retention-policy." + fmt); var el = document.getElementById("rp-export-result"); if(el) el.innerHTML = "<div style=\\"color:#66bb6a\\">Policy exported.</div>"; });'
      + '      });'
      + '      break;'
      + '  }'
      + '}';

    var fullHipaaAdminHtml = '<!DOCTYPE html><html><head><meta charset="utf-8"><style>'
      + hipaaAdminCss + '</style></head><body><div id="hipaa-app">'
      + hipaaAdminHtml + '</div><script>'
      + hipaaAdminJs + '</' + 'script></body></html>';
    return HtmlService.createHtmlOutput(fullHipaaAdminHtml)
      .setTitle('HIPAA Admin')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  ...
  var html = `
    ...
    <head>
      ...
      <style>
        ...
        #admin-badge { position: fixed; top: 12px; right: 12px; z-index: 100; background: rgba(255,255,255,0.12); padding: 4px 10px; border-radius: 16px; font: 10px/1 monospace; text-transform: uppercase; letter-spacing: 0.5px; color: #90caf9; cursor: pointer; user-select: none; }
        #admin-badge:hover { background: rgba(255,255,255,0.2); color: #fff; }
        #admin-dropdown-gas { display: none; position: fixed; top: 36px; right: 12px; z-index: 101; background: rgba(20,20,30,0.95); border: 1px solid rgba(255,255,255,0.15); border-radius: 6px; padding: 4px 0; min-width: 140px; box-shadow: 0 4px 16px rgba(0,0,0,0.4); }
        ...
        #admin-panel { position: fixed; top: 40px; right: 8px; z-index: 201; background: rgba(20,20,30,0.98); color: #ccc; border: 1px solid #444; border-radius: 8px; font: 12px/1.4 monospace; width: 480px; max-width: calc(100vw - 16px); max-height: calc(100vh - 80px); overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.5); display: flex; flex-direction: column; }
...
// ═══════════════════════════════════════════════════════
// HIPAA COMPLIANCE — Admin Panel Menu (served via hipaaAdmin iframe)
// ═══════════════════════════════════════════════════════

/**
 * Returns the list of HIPAA menu items the current user has permission to see.
 * Called by the hipaaAdmin iframe via google.script.run after receiving the session token.
 * @param {string} sessionToken - The session token from the parent page
 * @return {Object} { success, items: [{id, label, phase, permission, description}] }
 */
function getHipaaMenu(sessionToken) {
  var user = validateSessionForData(sessionToken, 'getHipaaMenu');
  var allItems = [
    { id: 'disclosures', label: 'Disclosures', phase: 'A', permission: 'read', description: 'View disclosure history' },
    { id: 'data-export', label: 'My Data', phase: 'A', permission: 'export', description: 'Download your data' },
    { id: 'amendment', label: 'Correction', phase: 'A', permission: 'amend', description: 'Request a record correction' },
    { id: 'amendment-review', label: 'Amendments', phase: 'A', permission: 'admin', description: 'Review pending amendments' },
    { id: 'disagreement', label: 'Disagree', phase: 'A', permission: 'amend', description: 'File a disagreement statement' },
    { id: 'extension', label: 'Extensions', phase: 'A', permission: 'admin', description: 'Grant 30-day extension' },
    { id: 'denial-notice', label: 'Denial Notice', phase: 'A', permission: 'admin', description: 'Generate formal denial' },
    { id: 'ehr-disclosures', label: 'EHR Disclosures', phase: 'A', permission: 'read', description: 'HITECH EHR accounting' },
    { id: 'breach-log', label: 'Breach Log', phase: 'B', permission: 'admin', description: 'Breach detection & logging' },
    { id: 'representatives', label: 'Representatives', phase: 'B', permission: 'admin', description: 'Personal representatives' },
    { id: 'legal-holds', label: 'Legal Holds', phase: 'C', permission: 'admin', description: 'Manage litigation holds' },
    { id: 'compliance-audit', label: 'Compliance Audit', phase: 'C', permission: 'admin', description: 'Retention compliance audit' },
    { id: 'archive-integrity', label: 'Archive Integrity', phase: 'C', permission: 'admin', description: 'Verify archive integrity' },
    { id: 'retention-policy', label: 'Retention Policy', phase: 'C', permission: 'admin', description: 'Retention policy docs' }
  ];
  var permitted = [];
  for (var i = 0; i < allItems.length; i++) {
    if (hasPermission(user.role, allItems[i].permission)) {
      permitted.push(allItems[i]);
    }
  }
  return { success: true, items: permitted };
}
