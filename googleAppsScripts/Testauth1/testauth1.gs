...
function doGet(e) {
  ...
  if (session.status !== "authorized") {
    ...
    var handshakeHtml = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body><script>'
      ...
      // Challenge the parent — ask for session token via postMessage
      ...
      + '}, PARENT_ORIGIN);'
      // Listen for handshake response with session token
      + 'window.addEventListener("message", function(evt) {'
      + '  if (evt.origin !== PARENT_ORIGIN) return;'
      ...
      + '    return;'
      + '  }'
      // Generate one-time nonce server-side
      + '  google.script.run'
      + '    .withSuccessHandler(function(r) {'
      ...
      + '});'
      // Timeout: if no parent responds in 5 seconds, this is direct access — show nothing
      + 'setTimeout(function() {'
      + '  if (!_challenged) {'
