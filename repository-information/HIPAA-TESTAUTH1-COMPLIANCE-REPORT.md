# HIPAA Compliance Report — testauth1 Environment
...
>
> This report evaluates every requirement from `HIPAA-CODING-REQUIREMENTS.md` against the actual testauth1 implementation, determining compliance status for each of the 40 checklist items plus underlying regulatory requirements.
...
| 🔄 **Not Applicable** | Requirement does not apply to the testauth1 architecture |
...
|--------|----------|-----------|
| Unique ID per user | Google OAuth email used as unique identifier | `testauth1.gs:594` — `email: userInfo.email` |
| No shared accounts | Each session tied to individual email | `testauth1.gs:593-603` — session data includes email, displayName |
| Tracking per action | Session token linked to email for all operations | `testauth1.gs:618` — audit log includes user email |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Identity verification | Google OAuth validates access token against Google's userinfo endpoint | `testauth1.gs:847-871` — `validateGoogleToken()` |
| Token exchange | OAuth token exchanged for server-side session token | `testauth1.gs:482-629` — `exchangeTokenForSession()` |
| Ongoing verification | Session validated on every heartbeat and data operation | `testauth1.gs:631-695` — `validateSession()` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Break-glass mechanism | Emergency access emails stored in Script Properties | `testauth1.gs:99` — `ENABLE_EMERGENCY_ACCESS: true` |
| Override logic | Emergency emails bypass spreadsheet ACL check | `testauth1.gs:921-934` — `checkSpreadsheetAccess()` emergency override |
| Heavy audit logging | Emergency access events logged to audit trail | `testauth1.gs:929` — `auditLog('emergency_access', ...)` |
| Data-level tracking | Emergency access flagged on every data operation | `testauth1.gs:768` — `isEmergencyAccess` in session data |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Inactivity timeout | Rolling session expiration (15min production / 3min test) | `testauth1.gs:80-81` — `SESSION_EXPIRATION: 900` (production) |
| Absolute timeout | Hard ceiling on session life regardless of activity (8hr production / 5min test) | `testauth1.gs:82-83` — `ABSOLUTE_SESSION_TIMEOUT: 28800` (production) |
| Server-side enforcement | Session destroyed in CacheService on timeout | `testauth1.gs:660-677` — timeout checks in `validateSession()` |
| Heartbeat extension | Active users get session extended; inactive users expire | `testauth1.gs:1067-1073` — `processHeartbeat()` resets `createdAt` |
| Client-side awareness | Timer display shows remaining session time | `testauth1.html:327` — heartbeat timer UI |
...
|--------|----------|--------|
| ACL-based access control | Master ACL spreadsheet with per-page TRUE/FALSE | ✅ `testauth1.gs:941-971` |
| Editor/viewer fallback | Spreadsheet sharing-list check | ✅ `testauth1.gs:974-992` |
| Domain restriction | Toggle-gated domain allowlist | ✅ `testauth1.gs:544-566` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Session invalidation | `invalidateAllSessions()` destroys all sessions for a user | `testauth1.gs:817-841` |
| Single-session enforcement | `MAX_SESSIONS_PER_USER: 1` — new login kills existing sessions | `testauth1.gs:580-583` |
| Cross-device eviction | Tombstone mechanism notifies other devices | `testauth1.gs:831-833` — `evicted_` cache keys |
...
**Finding:** The testauth1 environment delegates authentication to Google OAuth. If the user has Google 2-Step Verification enabled, they effectively have MFA. However, the app does not **require** the user to have 2FA enabled on their Google account — there is no enforcement. If the NPRM is finalized, you would need to either: (a) restrict access to Google Workspace accounts with enforced 2FA, or (b) add an application-level second factor.
...
|--------|--------|----------|
| Session data encryption | ⚠️ **Not encrypted** — stored as plaintext JSON in GAS CacheService | `testauth1.gs:611` — `cache.put(... JSON.stringify(sessionData) ...)` |
| Session integrity | ✅ HMAC-SHA256 protects against tampering | `testauth1.gs:606-608` — HMAC on session data |
...
| Token exchange (postMessage) | Token never transmitted via URL in HIPAA mode | ✅ `testauth1.gs:101` — `TOKEN_EXCHANGE_METHOD: 'postMessage'` |
...
| Message-level integrity | HMAC-SHA256 signatures on all postMessage communications | `testauth1.gs:453-475` — `signMessage()` |
| Client-side HMAC verification | Web Crypto API verifies HMAC-SHA256 signatures | `testauth1.html:1303-1320` — `_verifyHmacSha256()` |
| Non-extractable keys | HMAC key imported as non-extractable via Web Crypto API | `testauth1.html:1287-1299` — `_importHmacKey()` |
| First-write-wins key guard | Prevents HMAC key overwrite by attacker | `testauth1.html:1286` — `_hmacKeySet` flag |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Session audit log | `SessionAuditLog` sheet records all auth events | `testauth1.gs:334-356` — `_writeAuditLogEntry()` |
| Data audit log | `DataAuditLog` sheet records per-operation PHI access | `testauth1.gs:364-400` — `dataAuditLog()` |
| Toggle-gated | HIPAA preset enables both: `ENABLE_AUDIT_LOG: true`, `ENABLE_DATA_AUDIT_LOG: true` | `testauth1.gs:94,107` |
| Events logged | Login success/failure, session expiry, emergency access, security alerts, data reads/writes | Throughout `testauth1.gs` |
| Tamper resistance | Audit log sheet protected with warning-only lock | `testauth1.gs:342-344,384-386` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Failed login tracking | `login_failed` events logged with reason | `testauth1.gs:496,539,561,573` |
| Rate limiting (standard) | Flat: 5 failures per 5-minute window | `testauth1.gs:494-498` |
| Escalating lockout (HIPAA) | Tier 1: 5→5min, Tier 2: 10→30min, Tier 3: 20→6hr | `testauth1.gs:500-526` |
| Account lock alerts | `security_alert` logged on Tier 3 lockout | `testauth1.gs:512` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Incident detection | `security_alert` events for HMAC mismatches, missing secrets, suspicious activity | Throughout `testauth1.gs` |
| Client-side reporting | `_reportSecurityEvent()` sends blocked attacks to GAS backend | `testauth1.html:1232+` |
| Security event logging | Dedicated security event processing with rate limiting | `testauth1.gs:1100-1132` — `processSecurityEvent()` |
| Flood prevention | Max 50 security events per 5-minute window | `testauth1.gs:1104-1131` |
...
| Aspect | Evidence | Status |
|--------|----------|--------|
| Configured retention | `AUDIT_LOG_RETENTION_YEARS: 6` | ✅ `testauth1.gs:96` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| HMAC-SHA256 session integrity | Session data signed with HMAC on creation and verified on every access | `testauth1.gs:406-440` |
| Fail-closed design | Missing HMAC secret prevents session creation entirely | `testauth1.gs:410-416` — throws Error |
| Tampered session rejection | HMAC mismatch → session destroyed + security alert | `testauth1.gs:651-656` |
| Message integrity | All postMessage communications signed with per-session HMAC key | `testauth1.gs:453-475` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| HMAC verification on every access | `verifySessionHmac()` called in `validateSession()` and `validateSessionForData()` | `testauth1.gs:650-656,736-741` |
| Integrity violation detection | Unauthorized alteration detected and logged as `hmac_mismatch` | `testauth1.gs:652-653` |
| Client-side verification | Host page verifies HMAC signatures on incoming messages | `testauth1.html:1303-1320` |
...
| Aspect | Status | Evidence |
|--------|--------|----------|
| Session data | ✅ Only email and displayName stored — access token discarded after validation | `testauth1.gs:596-597` — comment: "accessToken intentionally NOT stored" |
| Audit log data | ✅ Session tokens truncated to 8 chars in logs | `testauth1.gs:791` — `sessionToken.substring(0, 8)` |
| API responses | ⚠️ `saveNote()` returns the full note text back to the client | `testauth1.gs:799` |
| **Field-level filtering** | ❌ No mechanism to return different data subsets based on role | No RBAC |
| **Data query scope** | 🔄 N/A — testauth1 is a test environment with minimal data operations | Only `saveNote()` exists |
...
**Mitigation:** Since testauth1 stores minimal data (only test notes in a spreadsheet), the right of access could currently be fulfilled manually (admin exports from Google Sheets). This would not scale.
...
**Finding:** Not applicable. The testauth1 environment does not de-identify data for research or secondary use. If de-identification is needed in the future, the 18-identifier stripping process documented in `HIPAA-CODING-REQUIREMENTS.md` Section 11 must be followed.
...
| Aspect | Status | Evidence |
|--------|--------|----------|
| Unauthorized access detection | ✅ Logged via `security_alert` events | Throughout `testauth1.gs` |
| HMAC integrity violation alerts | ✅ Detected and logged | `testauth1.gs:652-653` |
| Escalating lockout (brute force) | ✅ Tier 3 locks account for 6 hours | `testauth1.gs:511-514` |
| Client-side attack reporting | ✅ `_reportSecurityEvent()` sends to backend | `testauth1.html:1232+` |
...
| Aspect | Evidence | File:Line |
|--------|----------|-----------|
| Data-level audit trail | `DataAuditLog` links access events to specific records | `testauth1.gs:387-396` — user, action, resourceType, resourceId |
| Session-to-user mapping | Every data operation tied to authenticated user | `testauth1.gs:780` — `validateSessionForData()` |
| Emergency access flagging | Emergency access events separately identified | `testauth1.gs:395` — `IsEmergencyAccess` column |
...
|--------|-----------|-----------------|----------|
| `SESSION_EXPIRATION` | 180s (3min) | 900s (15min) | `testauth1.gs:81,82` |
| `ABSOLUTE_SESSION_TIMEOUT` | 300s (5min) | 28800s (8hr) | `testauth1.gs:83,84` |
| `HEARTBEAT_INTERVAL` | 30s | 300s (5min) | `testauth1.gs:86,87` |
| `OAUTH_TOKEN_LIFETIME` | 180s (3min) | 3600s (1hr) | `testauth1.gs:89,90` |
| `OAUTH_REFRESH_BUFFER` | 60s (1min) | 300s (5min) | `testauth1.gs:91,92` |
| `HEARTBEAT_INTERVAL` (HTML) | 30000ms | 300000ms (5min) | `testauth1.html:391` |
...
| Date | Version | Change |
|------|---------|--------|
| 2026-03-19 | 1.0 | Initial compliance assessment — all 40 checklist items evaluated against testauth1 codebase |
