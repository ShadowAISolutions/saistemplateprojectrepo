# inventorymanagement.html — GAS Integration Sequence Diagram (Auth)
...
sequenceDiagram
    participant Browser
    participant HTML as inventorymanagement.html
    participant HV as inventorymanagementhtml.version.txt
    participant GV as inventorymanagementgs.version.txt
    participant GAS as GAS Web App<br>(Apps Script)
    participant Google as Google OAuth
    participant CL as Changelogs

    Note over Browser,HTML: Page Load
    Browser->>HTML: Load inventorymanagement.html
    ...
    loop Every 10 seconds
        HTML->>HV: GET inventorymanagementhtml.version.txt?_cb=timestamp
    ...
    HTML->>GV: Initial check: does inventorymanagementgs.version.txt exist?
    GV-->>HTML: v01.XXg (exists → show GAS pill)
    loop Every 10 seconds (after 15s initial delay)
        HTML->>GV: GET inventorymanagementgs.version.txt?_cb=timestamp
        GV-->>HTML: v01.XXg
    ...
    HTML->>CL: Fetch inventorymanagementhtml.changelog.md
    CL-->>HTML: Markdown → parsed to HTML popup

    Browser->>HTML: Click GAS version pill
    HTML->>CL: Fetch inventorymanagementgs.changelog.md
    CL-->>HTML: Markdown → parsed to HTML popup
```
