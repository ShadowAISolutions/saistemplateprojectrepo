# rndlivedata.html — GAS Integration Sequence Diagram
...
sequenceDiagram
    participant Browser
    participant HTML as rndlivedata.html
    participant HV as rndlivedatahtml.version.txt
    participant GV as rndlivedatags.version.txt
    participant GAS as GAS Web App<br>(Apps Script)

    participant CL as Changelogs

    Note over Browser,HTML: Page Load
    Browser->>HTML: Load rndlivedata.html
    ...
    loop Every 10 seconds
        HTML->>HV: GET rndlivedatahtml.version.txt?_cb=timestamp
    ...
    HTML->>GV: Initial check: does rndlivedatags.version.txt exist?
    GV-->>HTML: v01.XXg (exists → show GAS pill)
    loop Every 10 seconds (after 15s initial delay)
        HTML->>GV: GET rndlivedatags.version.txt?_cb=timestamp
        GV-->>HTML: v01.XXg
    ...
    HTML->>CL: Fetch rndlivedatahtml.changelog.md
    CL-->>HTML: Markdown → parsed to HTML popup

    Browser->>HTML: Click GAS version pill
    HTML->>CL: Fetch rndlivedatags.changelog.md
    CL-->>HTML: Markdown → parsed to HTML popup
```
