# portal.html — GAS Integration Sequence Diagram (Auth)
...
sequenceDiagram
    participant Browser
    participant HTML as portal.html
    participant HV as portalhtml.version.txt
    participant GV as portalgs.version.txt
    participant GAS as GAS Web App<br>(Apps Script)
    participant Google as Google OAuth
    participant CL as Changelogs

    Note over Browser,HTML: Page Load
    Browser->>HTML: Load portal.html
    ...
    loop Every 10 seconds
        HTML->>HV: GET portalhtml.version.txt?_cb=timestamp
    ...
    HTML->>GV: Initial check: does portalgs.version.txt exist?
    GV-->>HTML: v01.XXg (exists → show GAS pill)
    loop Every 10 seconds (after 15s initial delay)
        HTML->>GV: GET portalgs.version.txt?_cb=timestamp
        GV-->>HTML: v01.XXg
    ...
    HTML->>CL: Fetch portalhtml.changelog.md
    CL-->>HTML: Markdown → parsed to HTML popup

    Browser->>HTML: Click GAS version pill
    HTML->>CL: Fetch portalgs.changelog.md
    CL-->>HTML: Markdown → parsed to HTML popup
```
