...
| `DEVELOPER_LOGO_URL` | https://www.shadowaisolutions.com/SAIS_Logo.png | HTML pages<br>and templates | URL to the developer's logo.<br>Independent of `YOUR_ORG_LOGO_URL` and `SPLASH_LOGO_URL`.<br>Replace with your own logo URL after forking |
...
>   - When `TEMPLATE_DEPLOY` = `On`: version bumps ARE performed — Pre-Commit items [PC-GS-VERSION] #1 (`.gs` version bump), [PC-HTML-VERSION] #2 (html.version.txt bump), [PC-HTML-SOURCE] #3 (html.version.txt single source), [PC-REPO-ARCH] #5 (REPO-ARCHITECTURE.md structural & behavioral changes), [PC-CHANGELOG] #6 (CHANGELOG.md), [PC-COMMIT-MSG] #8 (version prefix in commit message), [PC-REPO-VERSION] #15 (repo version bump), and [PC-PAGE-CHANGELOG] #16 (page changelog) all run normally, so that deployed pages auto-refresh after changes and changelogs track version history. Item **[PC-QR-CODE] #13 (QR code generation)** is still skipped regardless — QR codes use placeholder URLs on the template
...
5. **[PC-REPO-ARCH]** **REPO-ARCHITECTURE.md** — if the project structure changed (files or directories added, moved, or deleted) **or** if code was modified that affects the behavior depicted in any diagram (sequence diagrams, state diagrams, flowcharts, per-environment diagrams in `repository-information/diagrams/`), update the affected diagram(s) in `repository-information/REPO-ARCHITECTURE.md` and/or `repository-information/diagrams/`. **Behavioral trigger examples**: changing the auto-refresh polling logic in the HTML template → update the state diagram (section 3); changing the CI/CD workflow steps → update the flowchart (section 1) or sequence diagram (section 2); changing GAS script behavior → update the per-environment diagram and any REPO-ARCHITECTURE.md diagrams that show that behavior. **The test**: after completing edits, ask "does any diagram in the repo depict the behavior I just changed?" If yes, verify the diagram still matches the code and update it if not. This item applies on every repo including the template repo. **Version numbers are NOT shown in the diagram** — REPO-ARCHITECTURE.md is a structural overview, not a version dashboard. Mermaid nodes show filenames only (no version strings). **Template origin labels** — four-tier system: `[template]` (on the template repo — unmodified original; on forks — file that init did not modify), `[template · initialized]` (on forks — file modified by the init script during setup), `[template · modified]` (on forks — `[template]` file customized post-init), `[template · initialized · modified]` (on forks — `[template · initialized]` file customized post-init). File and directory nodes are prefixed with the appropriate label in their Mermaid label (e.g. `["[template] index.html"]`). File-centric subgraphs also carry the label in their title (e.g. `"live-site-pages/ — Hosted Content [template]"`). Process/workflow subgraphs (CI/CD steps, Auto-Refresh Loop, GAS Self-Update Loop, Developer Workflow, GitHub Pages Deployment) do NOT get the label — they represent runtime behavior, not file origins. New files added on a non-template repo do NOT get any label. On non-template repos, if a `[template · initialized]` node's underlying file is modified post-init, update its label to `[template · initialized · modified]`. If a `[template]` node's underlying file is modified post-init, update its label to `[template · modified]`. **mermaid.live link and copy block** — REPO-ARCHITECTURE.md includes an "Open in mermaid.live" link above the diagram (pako-compressed URL using the actual pako + js-base64 npm libraries) and a collapsible `<details>` section below the diagram with the raw code for manual copy-paste. **Whenever the diagram code changes**, regenerate both: (1) the URL — run the Node.js command below to generate it, then **capture the output to a variable or temp file** and use that to update the link (do NOT manually copy-paste or type the URL — the encoded string is ~2900 characters and the Edit tool can corrupt individual characters during replacement). The safest approach: write the output to `/tmp/mermaid_url.txt`, then read it back and use it in the Edit call. **Mandatory verification** — after updating the URL in the file, verify it decompresses correctly by running: `python3 -c "import base64,zlib,json,re; f=open('repository-information/REPO-ARCHITECTURE.md').read(); m=re.search(r'mermaid\.live/edit#pako:([^\)]+)',f); e=m.group(1); p=e+'='*((4-len(e)%4)%4); d=zlib.decompress(base64.urlsafe_b64decode(p)); j=json.loads(d); print('OK —',len(j['code']),'chars')"` — if this fails with a checksum or decompression error, the URL was corrupted during the Edit operation and must be regenerated. **Generation command** (requires `npm install pako js-base64` in `/tmp` — uses the exact same pako + js-base64 libraries as mermaid.live): `node -e "const pako=require('/tmp/node_modules/pako');const{fromUint8Array}=require('/tmp/node_modules/js-base64');const fs=require('fs');const f=fs.readFileSync('repository-information/REPO-ARCHITECTURE.md','utf8');const s=f.indexOf('\x60\x60\x60mermaid\\n')+'\x60\x60\x60mermaid\\n'.length;const e=f.indexOf('\\n\x60\x60\x60',s);const state={code:f.substring(s,e),mermaid:JSON.stringify({theme:'default'}),autoSync:true,updateDiagram:true};const compressed=pako.deflate(new TextEncoder().encode(JSON.stringify(state)),{level:9});console.log('https://mermaid.live/edit#pako:'+fromUint8Array(compressed,true))"` and update the link, (2) the `<details>` code block — replace the raw code inside it with the current diagram code (everything between ` ```mermaid ` and the closing ` ``` `)
6. **[PC-CHANGELOG]** **CHANGELOG.md** — every user-facing change must have an entry under `## [Unreleased]` in `repository-information/CHANGELOG.md`. The mega-rule below breaks into seven sub-steps; unless an individual sub-step says otherwise, all apply on every commit. **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`** — keep the file clean with `*(No changes yet)*` under a bare `## [Unreleased]` so forks start with a blank history.
  - **6a. Entry categories and format** — group entries under Keep a Changelog categories: `### Added` (new features), `### Changed` (changes to existing functionality), `### Fixed` (bug fixes), `### Deprecated` (soon-to-be removed features), `### Removed` (removed features), `### Security` (vulnerability fixes). Only include categories that have entries — no empty headings. Entry format: `- Description` (no per-entry timestamps — the version section header carries one timestamp for the whole push).
  - **6b. Per-file subheadings** — when a version section includes changes to files with their own dedicated changelogs (HTML pages in `live-site-pages/` or GAS scripts in `googleAppsScripts/`), add `#### \`filename\` — vXX.XXw` (or `vXX.XXg` for GAS) subheadings after the main category entries, showing the version that file became. Under each file subheading use `#####` category headings (`##### Added`, `##### Changed`, `##### Fixed`) and entries. If a change affects a file but does not bump its version, use `#### \`filename\` — vXX.XXw (no change)`. Only include file subheadings for files that were actually affected — repo-only changes (CLAUDE.md, README.md, rules files) do not get subheadings. When the same change applies identically to multiple files, each file still gets its own subheading with its own entries — do not combine them.
  - **6c. Push-commit versioning** — on the **push commit** (the final commit before `git push`), move all entries from `## [Unreleased]` into a new version section. The repo version [PC-REPO-VERSION] bumps on the push commit, so each push produces exactly one version section, not one per commit. Header format: `## [vXX.XXr] — YYYY-MM-DD HH:MM:SS AM/PM EST` — no commit SHA (SHAs are added during archive rotation; see CHANGELOG-archive.md "SHA enrichment"). **Timestamps must be real** — run `TZ=America/New_York date '+%Y-%m-%d %I:%M:%S %p EST'` to get the actual current time; never fabricate or increment. Latest versions appear first (reverse chronological). `## [Unreleased]` is then left empty (no placeholder — it will accumulate entries from the next session onward). **One version, one set of entries** — each version section contains only the entries for changes introduced by that specific push. Later pushes must NOT duplicate or re-include entries from prior version sections.
  - **6d. Prompt quote** — immediately after the version section header (before any category headings), include a blockquoted copy of the user's **full, unabridged** original prompt. Format: `> **Prompt:** "exact user prompt text here"`. **Never truncate, shorten, or abbreviate** — no ellipsis, no paraphrase, no summary. The entire prompt must appear verbatim. If multiple user prompts contributed to the same version (e.g. intermediate commits accumulated under `[Unreleased]`), use the prompt from the push commit's interaction.
  - **6e. Capacity counter** — the CHANGELOG header includes a `` `Sections: X/100` `` line showing the count of version sections vs. the rotation limit. Update this counter on every push commit after creating the new version section (and after any archive rotation). Count all `## [vXX.XX*]` lines excluding `## [Unreleased]`. **This counter is a mandatory rotation trigger — when it exceeds 100, archive rotation must run before the push.**
  - **6f. Archive rotation** — on the push commit, after creating the new version section, read the capacity counter and check if X > 100. If so, rotation is **MANDATORY** — do not skip. **Quick rule: 100 triggers, date groups move. A date group is ALL sections sharing the same date — could be 1 section or 500. Never split a date group. Today's sections (EST) are always exempt. At least one date group MUST be rotated when the trigger fires. Repeat until ≤100 non-exempt sections remain.** Full rotation logic lives in `repository-information/CHANGELOG-archive.md` (see "Rotation Logic"). **SHA enrichment is MANDATORY during rotation** — every section header moved to the archive must have a commit SHA link appended (see step 6 in CHANGELOG-archive.md). **Shallow clone fix** — before SHA lookups, run `git fetch --unshallow origin main 2>/dev/null || true` (Claude Code sessions often start with a shallow clone that hides older commits). If a version's commit genuinely cannot be found after unshallowing, mark with `— [SHA unavailable]`. **Post-rotation verification** — run `grep '^## \[v' ARCHIVE_FILE | grep -v '— \[' | head -5` — if any lines appear, SHA enrichment was missed; do not commit until this check passes.
  - **6g. Template-deploy reset and multi-session deferral** — **When `TEMPLATE_DEPLOY` switches from `On` → `Off`**, reset the CHANGELOG: remove all version sections, remove all entries and category headings under `## [Unreleased]`, replace with `*(No changes yet)*`, remove the date from the section header, and reset the counter to `` `Sections: 0/100` ``. **When `MULTI_SESSION_MODE` = `On`**: entries are still added to `## [Unreleased]` on every commit, but the push-commit versioned section creation is skipped — entries accumulate across all parallel sessions and are bundled into a single version section during reconciliation (see Reconcile Multi-Session Command).
7. **[PC-README-TREE]** **README.md structure tree** — if files or directories were added, moved, or deleted, update the ASCII tree in `README.md`. **Linked entry format** — the tree is inside an HTML `<pre>` block (not a fenced code block) so that every filename and directory name is a clickable `<a href>` link while preserving monospace formatting and line breaks. Files link to their blob-view: `<a href="https://github.com/ORG/REPO/blob/main/FULL_PATH">filename</a>`. Directories link to their tree-view: `<a href="https://github.com/ORG/REPO/tree/main/FULL_PATH">dirname/</a>`. Descriptions use `—` as separator. Entry format: `TREE_CHARS <a href="url">name</a> — [label] Description`. Use `&amp;` for literal `&` characters inside `<pre>` (HTML entity required). When adding new entries, follow this format to keep the tree navigable. Resolve `ORG` and `REPO` from `git remote -v`. **Group labels** — root-level files are organized under bold group labels that act as visual subheadings within the `<pre>` block. Format: `<b>─── Group Name ─────...─</b>` (using box-drawing characters padded to ~78 chars). Current groups: Configuration, Documentation, Community. When adding new root-level files, place them under the appropriate group. When adding new groups, follow the same format. Group labels replace what were previously separate `## Documentation` and `## Community` sections — the tree is now the single comprehensive file listing. **Completeness audit** — when verifying the tree matches the actual file structure, every directory that appears in the tree must have its contents fully expanded (listing each file inside it). A directory name alone (e.g. `└── sounds/`) does NOT count as "present" for the files it contains — each child file must be listed individually. Run `find` or `ls -R` and compare against the tree entry-by-entry, checking both top-level entries and subdirectory contents. **Template origin labels** — four-tier system: `[template]` (on the template repo — unmodified original; on forks — file that init did not modify), `[template · initialized]` (on forks — file modified by the init script during setup), `[template · modified]` (on forks — `[template]` file customized post-init), `[template · initialized · modified]` (on forks — `[template · initialized]` file customized post-init). Files that originated from the template repo are labeled after the `—` separator (e.g. `— [template] Live landing page`). New files added on a non-template repo do NOT get any label — their absence of a label signals they were created in the fork. On non-template repos, if a `[template · initialized]` file is modified in a post-init commit, update its label to `[template · initialized · modified]`. If a `[template]` file (one that init did not touch) is modified post-init, update its label to `[template · modified]`. This tells the developer both the file's init history and whether it was customized. On the template repo itself, labels always stay as `[template]` — modifications are just template updates, not customizations. **Page version display** — each live site page entry in the tree shows its current HTML and GAS versions as clickable links to their changelogs, placed between the first `—` separator and the description, with a `|` separating versions from the description. Format: `— <a href="URL_TO_HTML_CHANGELOG">vXX.XXw</a> · <a href="URL_TO_GAS_CHANGELOG">vXX.XXg</a> | [label] Description`. Pages without a GAS project use `vNoGASg` as a placeholder for the GAS version position. When [PC-GS-VERSION] #1 (GAS version bump) or [PC-HTML-VERSION] #2 (HTML version bump) fires, also update the corresponding version display in the README tree to match the new version. The `setup-gas-project.sh` script should add the version display when creating new page entries. On forks, the changelog URLs use the fork's org/repo. On the template repo, they use `ShadowAISolutions/saistemplateprojectrepo`
...
16. **[PC-PAGE-CHANGELOG]** **Page & GAS changelogs** — if any embedding HTML page in `live-site-pages/` or any `.gs` file in `googleAppsScripts/` was modified, add a user-facing entry to the corresponding changelog: `<page-name>html.changelog.md` in `live-site-pages/html-changelogs/` for pages, `<page-name>gs.changelog.md` in `live-site-pages/gs-changelogs/` for GAS scripts. These changelogs are the **single source of truth** — they live directly in the deployed directory and are fetched by the live site's changelog popup (no separate deployment copy needed). **Structurally this rule follows [PC-CHANGELOG] sub-steps 6a–6g** with the deltas below:
  - **User-facing writing style** (unlike the developer-facing repo CHANGELOG) — describe what a visitor/user experiences. "Faster page loading" not "Optimized database queries". "Fixed login button not responding on mobile" not "Fixed event listener delegation in auth.js". Never mention file names, function names, commit SHAs, deployment IDs, or internal architecture. Full security rules (HIPAA/PHI sanitization, no implementation detail) live in `.claude/rules/changelogs.md` — section "Changelog Security"
  - **Version section header format** — page changelogs use `## [vXX.XXw] — YYYY-MM-DD HH:MM:SS AM/PM EST — vXX.XXr` (page version + repo version cross-reference). GAS changelogs use `## [vXX.XXg] — YYYY-MM-DD HH:MM:SS AM/PM EST — vXX.XXr`. The repo version after the timestamp lets the developer trace which push introduced the change
  - **Every version bump gets an entry** — if the HTML/GAS change is purely internal (e.g. refactoring with no visible effect), still add `- Minor internal improvements` under `### Changed`. No version exists without at least one entry — the generic entry keeps version continuity without exposing internals
  - **Archive rotation destinations** — `<page-name>html.changelog-archive.md` or `<page-name>gs.changelog-archive.md`, in the same `live-site-pages/` subdirectory as the changelog. Same 100-section rotation and SHA-enrichment rules as 6f
  - **Public deployment** — these files are fetched by the live site's changelog popup over HTTPS. Any entry that leaks internal detail, PHI, or security posture is publicly visible. The repo CHANGELOG [PC-CHANGELOG] is exempt from the public-safety restrictions because it lives in `repository-information/` which is not deployed — this rule is not
  - **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`** — keep changelogs clean with `*(No changes yet)*` under `## [Unreleased]`. When `TEMPLATE_DEPLOY` switches from `On` → `Off`, reset all page and GAS changelogs and their archives the same way as 6g
  - **Multi-session deferral** — when `MULTI_SESSION_MODE` = `On`, entries still land in `## [Unreleased]`; versioned-section creation is deferred to reconciliation (same as 6g)
...
5. **Push-once enforcement** — verify that the `claude/*` branch does not currently exist on the remote. The auto-merge workflow deletes the branch after merging into `main`; if the branch still exists, a prior workflow run is still in flight and a second push would queue a colliding workflow. Run the following sub-steps in order:
   - **5a. Initial check** — run `git ls-remote origin <branch-name>`. If the output is empty, the branch does not exist on the remote → pushing is safe (skip to 5d).
   - **5b. Retry with exponential backoff** — if 5a shows the branch still exists, the workflow hasn't finished yet. Wait and re-check up to 4 times with exponential backoff: wait 2s, re-check; wait 4s, re-check; wait 8s, re-check; wait 16s, re-check (total max wait ~30s). If any retry returns empty, pushing is safe (skip to 5d).
   - **5c. Still-exists handling** — if the branch **still exists after all 4 retries**, treat it as one of two cases and decide:
     - **Case A — workflow in flight**: default assumption. Flag `🚧🚧BLOCKED🚧🚧` to the user explaining push-once enforcement (the prior push's workflow is still running; a second push would queue a colliding run). Do not push until the branch is gone from the remote.
     - **Case B — failed workflow recovery**: the prior workflow failed and the branch is stuck on the remote without being merged. In this case, a re-push to the same branch is the correct recovery path — it re-triggers the workflow. Confirm with the user before re-pushing into a stuck branch.
   - **5d. Safe to push** — the branch does not exist on the remote. Proceed with `git push -u origin <branch-name>` per the Git Operations rules (retry on network failures only, up to 4 times with 2s/4s/8s/16s backoff).
...
3. **No version bumps for new files** — the script creates files at initial versions (`|v01.00w|`, `|v01.00g|`). Do NOT bump versions for newly created files. Only bump if you make additional edits to them (e.g. fixing a warning)
...
**This is a mechanical execution** — the setup script does all the heavy lifting. Claude's role is: run script → check output → checklist → commit → push. No manual REPO-ARCHITECTURE.md edits, no manual README tree edits, no manual gas-scripts.md edits.
...
1. **Determine template source** — check if a `template` remote exists (`git remote -v | grep template`). If not, add it: `git remote add template https://github.com/ShadowAISolutions/saistemplateprojectrepo.git`. Fetch the latest: `git fetch template main`
...
══════════════════════════════
  ...
  Template: ShadowAISolutions/saistemplateprojectrepo
══════════════════════════════
...
|-------|-----------|-------------|
| Version Bumping (.gs) | `.claude/rules/gas-scripts.md` | [PC-GS-VERSION] #1 |
| Build Version (HTML polling, maintenance mode, new page setup) | `.claude/rules/html-pages.md` | [PC-HTML-VERSION] #2, [PC-HTML-SOURCE] #3, [PC-TEMPLATE-FREEZE] #4 |
| Commit Message Naming | `.claude/rules/gas-scripts.md` | [PC-COMMIT-MSG] #8 |
| REPO-ARCHITECTURE.md Structural & Behavioral Updates | `.claude/rules/repo-docs.md` | [PC-REPO-ARCH] #5 |
| GAS Project Config (config.json) | `.claude/rules/gas-scripts.md` | [PC-GAS-CONFIG] #14 |
| Keeping Documentation Files in Sync | `.claude/rules/repo-docs.md` | [PC-REPO-ARCH] #5, [PC-CHANGELOG] #6, [PC-README-TREE] #7 |
| Internal Link Reference | `.claude/rules/repo-docs.md` | [PC-LINKS] #11 |
...
| Changelog Formats & Archive Rotation | `.claude/rules/changelogs.md` | [PC-CHANGELOG] #6, [PC-PAGE-CHANGELOG] #16 |
| Private Repo Compatibility | `.claude/rules/html-pages.md` | [PC-PRIVATE-REPO] #18 |
| Template Source Propagation | `.claude/rules/html-pages.md` | [PC-TEMPLATE-PROP] #19 |
