...
| `DEVELOPER_LOGO_URL` | https://www.shadowaisolutions.com/SAIS_Logo.png | HTML splash screen<br>`SPLASH_LOGO_URL` variable<br>(in pages<br>and templates) | URL to the developer's logo.<br>`SPLASH_LOGO_URL` defaults to this value when no separate splash logo is set.<br>Replace with your own logo URL after forking |
...
> - Pre-Commit items #0, #4, #5, #7, #9, #10, #11, #12, #13, #14, #17, #18, #19 still apply normally (plus #1, #2, #3, #6, #8, #15, #16 when `TEMPLATE_DEPLOY` = `On`)
...
> - **#15** (repo version bump): **skipped entirely** — `repository.version.txt` is not touched
> - **#10** (README `Last updated:` line): **skipped entirely** — the timestamp/version line is not updated
> - **#6** (CHANGELOG.md): entries are still added to `## [Unreleased]` on every commit, but the **push-commit versioned section creation is skipped** — entries accumulate under `[Unreleased]` across all sessions and are versioned during reconciliation
> - **#16** (page & GAS changelogs): same as #6 — entries added to `[Unreleased]`, versioned sections deferred
> - > - **#8** (commit message format): **no repo version prefix** — commit messages use plain descriptive text or `g`/`w` prefixes only (since per-page/GAS versions still bump)
>
> Items that **still run normally**: #0, #1, #2, #3, #4, #5, #7, #9, #11, #12, #13, #14, #17, #18. Per-page (#2) and per-GAS (#1) version bumps are safe because they're scoped to the specific file being edited — different sessions working on different pages won't conflict.
>
...
11. **Internal link integrity** — if any markdown file is added, moved, or renamed, verify that all internal links (`[text](path)`) in the repo still resolve to existing files. Pay special attention to cross-directory links — see the Internal Link Reference section for the correct relative paths
...
## CLAUDE.md
➕ Fork-added: [N items]
  - Pre-Commit #21: "New rule description" (lines XX-XX)
  - Behavioral rule: "New rule name" (lines XX-XX)
✏️ Fork-modified: [N items]
  - Pre-Commit #7: Changed CHANGELOG heuristic from ~10s to ~15s
...
| REPO-ARCHITECTURE.md Structural & Behavioral Updates | `.claude/rules/repo-docs.md` | #6 |
| GAS Project Config (config.json) | `.claude/rules/gas-scripts.md` | #15 |
| Keeping Documentation Files in Sync | `.claude/rules/repo-docs.md` | #5, #6, #7 |
