...
5. **STATUS.md** — if any version was bumped, update the matching version in `repository-information/STATUS.md`. **Template origin labels** — every row in the STATUS.md tables has an `Origin` column with three possible values: `template` (on the template repo — unmodified original), `initialized` (on forks — set by the init script during setup), or empty (fork-added entries). On non-template repos, if a row's underlying file is modified after initialization, do NOT change the Origin column — it tracks where the entry came from, not whether it was edited (the README tree and ARCHITECTURE.md `[template · modified]` labels handle that). **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`**. **Skip when `MULTI_SESSION_MODE` = `On`**
